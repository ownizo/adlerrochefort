...
// the real sweep against these eleven pages in an isolated worktree, with
// this exclusion lifted, produces zero duplication. It also confirmed what
// the bug had been accidentally protecting — with the exact-string blind
// spot gone, the sweep's footer replacement (which, unlike topBar's, has no
// content gate) happily overwrites the cluster's own footer with the
// sitewide one. That is the reason recorded here now: these eleven pages
// are excluded because their chrome is deliberately not the sitewide one,
// not because of a detection bug that no longer exists. /nl/index.html is
// not in this list because it is already a SOURCE.
