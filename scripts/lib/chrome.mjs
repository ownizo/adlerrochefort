...
// Pure builder, split out from writeSharedStylesheet() so a read-only
// consumer (scripts/check-generator-freshness.mjs) can ask "what would this
// produce right now" without writing anything — the freshness checker
// compares this against the committed public/css/ar-site.css instead of
// generating a file, diffing it, then having to restore the original.
export function buildSharedStylesheet() {
  return (
