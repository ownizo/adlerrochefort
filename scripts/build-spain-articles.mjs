...
<footer>
  <div class="footer-top">
    <div>
      ...
      <p class="footer-brand-desc">English-speaking insurance intermediary, registered with Portugal's ASF. We help international residents and property owners in Portugal and Spain work out what cover they need, and arrange it in English.</p>
    </div>
    <div>
...
// Situation-based acquisition (brief §5/§30): most of these articles touch
// more than one insurance category, so the primary conversion route is the
// multi-product review, not a single commercial page — the related-card
// grid below still routes to the specific products for a visitor who
// already knows which one they need.
function ctaHtml(a) {
  // a.situation (brief §31/§32): one of the fixed values the review form's
  // hidden entry_situation field recognises — set only on the four articles
  // this phase built or upgraded around a specific situation. Every other
  // article's CTA omits the parameter rather than guessing a value.
  const qs = a.situation ? `?market=Spain&situation=${encodeURIComponent(a.situation)}` : '?market=Spain';
  return `  <div class="article-cta">
    <p>If your situation touches more than one of these, tell us once rather than filling in several separate forms.</p>
    <a href="/en/insurance-review/${qs}">Review Your Insurance Needs &rarr;</a>
  </div>`;
}
